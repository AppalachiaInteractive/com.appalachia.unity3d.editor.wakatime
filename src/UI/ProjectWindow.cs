...
namespace Appalachia.WakaTime.UI
{
    ...
    {
        private static ProjectWindow _window;
        ...
        public static void Display()
        {
            if (_window)
            {
                FocusWindowIfItsOpen<ProjectWindow>();
            }
            else
            {
                _window = CreateInstance<ProjectWindow>();
                _window.ShowPopup();
            }

            // We need only first 2 lines from .wakatime-project
            _projectSettings = new[] {"", ""};
            var projectFile = WakaTime.GetProjectFile();
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                EditorGUILayout.SelectableLabel(_branch, RichHelpBoxStyle, GUILayout.Height(EditorGUIUtility.singleLineHeight));
            }
            EditorGUILayout.EndHorizontal();
            ...
            {
                GUILayout.Label($"<b>{Path.GetFullPath(".wakatime_project")}</b> will be created on save", RichHelpBoxStyle);
            }
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                {
                    WakaTime.SetProjectFile(_projectSettings);
                    WakaTime.Initialize();
                    CloseAndNull();
